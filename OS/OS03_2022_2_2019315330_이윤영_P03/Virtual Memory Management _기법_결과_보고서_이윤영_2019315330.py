    ...
    while True:
        ...
        if incomingPage in memory:
            ...
            if len(memory) < pageFrameNum:  # 메모리가 비어있는 첫 시작
                ...
                for i in memory:
                    if i not in refStringMin:  # 메모리에 page가 infinite fd 를 가질때 첫번째를 바로 victim 선정
                        memory.remove(i)
                        memory.append(incomingPage)
                        k = 1
                if k == 1:
...
def WS_algorithm():
    ...
    while True:
        ...
        refHistory.append(incomingPage)
        if len(refHistory) > windowSize + 1:  # delta 3일때 최근 referenced page가 4가 넘으면 첫번째 page 삭제
            refHistory.pop(0)
        if incomingPage in memory:  # 해당 page가 메모리에 있으면 pass
            n += 1
            continue
        elif incomingPage not in memory:  # 메모리에 해당 페이지가 없을 경우 page fault 발생
            pageFault.append(incomingPage)
            pageFaultIndex.append(n)
            memory.append(incomingPage)
            for i in memory:
                if i not in refHistory:
                    memory.remove(i)  # 메모리에 있는 page가 refHistory에 없으면 삭제
        n += 1
